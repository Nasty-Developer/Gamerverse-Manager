...
import { Search as SearchIcon, Filter, SlidersHorizontal, ChevronDown, ChevronUp, X } from "lucide-react";
...
export default function Search() {
  ...
  const selectGenre = (slug: string) => {
    ...
    if (next) {
      setLocation(`/search?q=${encodeURIComponent(query)}&genres=${next}`);
    }
  };
  ...
  return (
    <div className="container mx-auto px-4 py-8 min-h-screen">
      ...
      <form onSubmit={handleSearch} className="relative flex items-center mb-6">
        ...
        />
        <Button type="submit" className="absolute right-2 h-10 px-6 font-semibold">
          Search
        </Button>
      </form>
      ...
      <div className="md:hidden mb-4">
        ...
        >
          <span className="flex items-center gap-2 text-sm font-medium">
            ...
            Filters
            {selectedGenre && (
              <span className="bg-primary text-white text-xs px-2 py-0.5 rounded-full font-bold">1</span>
            )}
          </span>
        ...
        {filtersOpen && (
          <div className="mt-2 bg-card border border-white/10 rounded-xl p-4 space-y-4">
            <div>
              <h3 className="text-xs font-bold uppercase tracking-wider text-muted-foreground mb-3">Genres</h3>
            ...
            {selectedGenre && (
              <Button variant="ghost" size="sm" onClick={() => setSelectedGenre("")} className="text-muted-foreground hover:text-white w-full">
        ...
        <aside className="hidden md:block w-56 shrink-0 space-y-8">
          <div className="bg-card border border-white/10 rounded-xl p-5 sticky top-24">
            ...
            <div className="space-y-5">
              <div>
                <h3 className="text-xs font-bold text-muted-foreground mb-3 uppercase tracking-wider">Genres</h3>
              ...
              <div>
                <h3 className="text-xs font-bold text-muted-foreground mb-3 uppercase tracking-wider">Platforms</h3>
        ...
        <main className="flex-1 min-w-0 space-y-5">
          {activeQuery && (
            <div className="flex items-center justify-between bg-white/5 border border-white/10 rounded-xl p-4">
              <div>
                <h1 className="text-xl font-bold text-white">Results for "{activeQuery}"</h1>
                {searchResults?.intent && !isLoading && !isFetching && (
                  <p className="text-primary font-medium mt-0.5 text-sm">Intent: {searchResults.intent}</p>
                )}
              </div>
              <div className="text-muted-foreground font-medium text-sm shrink-0 ml-3">
                {isLoading || isFetching ? (
                  ...
                  <span>{searchResults?.count ? searchResults.count.toLocaleString() : 0} found</span>
                )}
              </div>
            </div>
          )}
          ...
          {isLoading || isFetching ? (
            ...
            <div className="text-center py-20 bg-card border border-white/10 rounded-xl">
              ...
              <p className="text-muted-foreground max-w-sm mx-auto text-sm">
                Nothing matched "{activeQuery}". Try different keywords or browse by genre.
              </p>
              <Button
                variant="outline"
                className="mt-6 border-white/10 hover:bg-white/5"
                onClick={() => { setQuery(""); setActiveQuery(""); setLocation("/search"); }}
              >
                Clear search
              </Button>
            </div>
            ...
            <div className="text-center py-20 bg-card border border-white/10 rounded-xl">
              ...
              <p className="text-muted-foreground max-w-sm mx-auto text-sm">
                Enter a search term above to explore our database of thousands of games.
              </p>
              <div className="mt-6 flex flex-wrap justify-center gap-2 max-w-sm mx-auto">
                {["RPG", "Action", "Indie", "Strategy", "Horror", "Open World"].map(term => (
                  <button
                    key={term}
                    onClick={() => { setQuery(term); setActiveQuery(term); setLocation(`/search?q=${encodeURIComponent(term)}`); }}
